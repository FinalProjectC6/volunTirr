...
console.log(userID);
  ...
  const [setMydataa, setMydata] = useState({
    phone: "",
    age: "",
    gender: "",
  });
  ...
  const info = async () => {
    try {
      const response = await axios.put(`http://192.168.101.3:3000/provider/updateprovider/${userID}`, setMydataa); // Include userID in the URL
      console.log("info updated:", response.data);
      navigation.navigate("packagee");
    } catch (error) {
  ...
  const handleGo = async () => {
    if ( !setMydataa.phone || !setMydataa.age || !setMydataa.gender ) {
      ...
      <View style={styles.allInput}>
        ...
        <View style={styles.inputContainer}>
          <PaperTextInput
            ...
            onChangeText={(text) => setMydata({ ...setMydataa, age: text })}
          ...
          <PaperTextInput
            ...
            keyboardType="numeric"
            onChangeText={(text) => setMydata({ ...setMydataa, phone: text })}
        ...
        <View style={styles.nameInput}>
          ...
          style={styles.input} 
          placeholder="Gender"
          onChangeText={(text) => setMydata({ ...setMydataa, gender: text })}
          />
        </View>
        </View>
