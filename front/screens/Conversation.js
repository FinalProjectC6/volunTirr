...
const Conversation = ({ route }) => {
  ...
  const sendMessage = async () => {
    ...
    try {
      await fetch(`http://192.168.100.10:3000/chat/createmessage`, {
        method: "POST",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.10:3000/chat/getallmessage/${ChatId}`)
  ...
  const startRecording = async () => {
    ...
    const stopRecording = async () => {
        ...
        fr.onloadend = async () =>
          await fetch(`http://192.168.100.10:3000/chat/createaudio/11`, {
            method: "POST",
            body: fr.result,
          });
        ...
        const base64Data = await fetch(
          `http://192.168.100.10:3000/chat/getaudio/11`
        ).then((res) => res.json());
